...
#' @param rast Optional user-supplied \code{SpatRaster} of seafloor depth
...
#' terra::values(r) <- c(50, 1500, 0, 3000)   # metres, land = 0
...
bathy_source <- function(resolution = 4, cache_dir = NULL,
  ...
  if (!is.null(rast) && !inherits(rast, "SpatRaster")) {
    stop("`rast` must be a SpatRaster of seafloor depth (positive-down)")
  }
  ...
  fetch <- function(lon, lat) {
    ...
    r <- terra::rast(marmap::as.raster(b))  # elevation: negative = depth, positive = land
...
#' \code{SpatRaster} used for all dates, e.g. a climatology, or a named list of
#' \code{SpatRaster}s keyed by \code{"YYYY-MM-DD"}) to work offline or use a
#' different product.
#'
...
#'
#' @section What the tag is actually measuring:
#' The hard part of an SST term is not the reference field, it is the tag value
#' you match against it. A thermistor mounted on an animal reads the animal
#' about as often as it reads the sea, and the contamination is one-sided, so it
#' does not average out.
#'
#' Measured on ten northern elephant seal deployments against OISST at Argos
#' positions, by the depth at which the reading was taken:
#'
#' \tabular{lrr}{
#'   \strong{band} \tab \strong{bias (deg C)} \tab \strong{sd} \cr
#'   hauled out    \tab +5.89 (max +23.6)     \tab -    \cr
#'   0 to 0.5 m    \tab +0.46                 \tab 2.59 \cr
#'   2 to 10 m     \tab -0.05                 \tab 0.44 to 0.72
#' }
#'
#' So the same instrument is an excellent SST sensor when submerged but shallow,
#' and unusable at the surface, which is the opposite of the intuition that
#' surface readings are the SST-like ones. Select on a depth BAND, never on
#' "shallowest". Reducing each knot with a high quantile is worse than useless:
#' it selects the warmest and therefore most contaminated reading, and on these
#' animals that made latitude error worse rather than better.
#'
#' A probe on a lead, away from the body, removes most of this. Even then, treat
#' SST as a CONSTRAINT rather than an accuracy term: an ocean isotherm is a broad
#' band, so a good SST match rules locations out without pinning one down, and in
#' a hemisphere-ambiguous season it can rule out the correct band. That is the
#' same job [bathy_source()] does with [floor_rule()], and it is better encoded
#' the same way.
#'
...
#' @param rast Optional offline data: a single \code{SpatRaster} (used for all
#'   dates) or a named list of \code{SpatRaster}s keyed by `"YYYY-MM-DD"`. If
...
#' r <- terra::rast(nrows = 2, ncols = 2,
#'                  xmin = 149, xmax = 151, ymin = -46, ymax = -44)
#' terra::values(r) <- c(11.2, 11.8, 12.1, 12.6)
...
sst_source <- function(dataset = "ncdcOisst21Agg_LonPM180",
  ...
  if (!is.null(rast) &&
    ...
    stop("`rast` must be a SpatRaster or a non-empty named list of SpatRasters")
  }

  state <- new.env(parent = emptyenv())
  state$static <- if (inherits(rast, "SpatRaster")) rast else NULL
  state$layers <- if (is.list(rast) && !inherits(rast, "SpatRaster")) rast else list()
  state$info   <- NULL
  ...
  fetch_day <- function(day, lon, lat) {
    ...
    terra::rast(df[, c("longitude", "latitude", field)], type = "xyz",
                crs = "EPSG:4326")
  }
...
#'
#' A thin adapter that turns a \code{SpatRaster} (static, date-independent) or
#' a named list of \code{SpatRaster}s (keyed by \code{"YYYY-MM-DD"}) into a
...
#'
#' @param r A \code{SpatRaster} (used for every date) or a non-empty named list
#'   of \code{SpatRaster}s keyed by \code{"YYYY-MM-DD"} strings.
#'
#' @return A source closure of class `tf_source`: `function(lon, lat, date)`
...
#' terra::values(r) <- c(5.0, 10.0)
#' rs <- raster_source(r)
#' rs(lon = c(0.5, 1.5), lat = c(0.5, 0.5), date = Sys.Date())
#' @export
raster_source <- function(r) {
  is_layer <- inherits(r, "SpatRaster")
  is_list  <- is.list(r) && length(r) > 0L && !is_layer
  if (!is_layer && !is_list) {
    stop("`r` must be a SpatRaster or a non-empty named list of SpatRasters")
  }
  src <- function(lon, lat, date = NULL) {
    if (is_layer) {
...
#'
#' @param r A \code{SpatRaster} whose values are log-prior densities (if
...
#' r <- terra::rast(nrows = 1, ncols = 2,
#'                  xmin = 0, xmax = 2, ymin = 0, ymax = 1)
#' terra::values(r) <- c(log(0.8), log(0.2))   # log-prior
#' ps <- prior_raster(r)
...
sea_mask_source <- function() {
  ...
  build <- function(lon, lat) {
    r <- terra::rast(
      xmin = min(lon) - 1, xmax = max(lon) + 1,
      ymin = min(lat) - 1, ymax = max(lat) + 1,
    ...
    sea_r <- land_r
    terra::values(sea_r) <- ifelse(terra::values(land_r) == 1, 0, 1)
    state$rast <- sea_r
    invisible(sea_r)
  }
