...
grid_tuning$RMSE <- sapply(1:nrow(grid_tuning), function(i) {
  grid <- makeGrid(lon=c(130, 155), lat=c(-50, -30),
  ...
  df <- data.frame(
    Time = as.POSIXct(fit$fit$time, origin="1970-01-01", tz="UTC"),
    Lon = fit$fit$lon,
    Lat = fit$fit$lat
  )
...
```{r comparison_easy, eval = FALSE}
# FLightR and SGAT comparison requires the original TwilightFree package
# (not on CRAN) for twilight extraction. See benchmark_comparison.Rmd for a
# full comparison using pre-computed caches.
```
...
grid_best_obj <- makeGrid(lon=c(130, 155), lat=c(-50, -30),
...
df_grid_best <- data.frame(
  Time = as.POSIXct(fit_grid_best$fit$time, origin="1970-01-01", tz="UTC"),
  Lon = fit_grid_best$fit$lon,
  Lat = fit_grid_best$fit$lat
)
