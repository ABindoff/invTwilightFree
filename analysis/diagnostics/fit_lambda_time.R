...
OUT <- "scratch/nes_calibration/bias_lambda_time.csv"

STEP_HOURS <- 12; CAL_DAYS <- 15
LATS   <- seq(20, 70, by = 0.5)
MAXWIN <- 60   # more windows: the question is now WITHIN-deployment variation
SHADE_RATIO <- 2
      ...
      for (lm in LAMS) {
        ...
        for (bn in names(bands)) {
          ...
          R[[length(R) + 1]] <- data.table(id = tg, delta = dl, lam_mult = lm,
                                           ...
                                           # WHEN, so lambda can be tested for
                                           # stationarity within a deployment. The
                                           # earlier sweeps aggregated this away and
                                           # so could only ever produce a per-tag
                                           # constant -- which assumes the thing
                                           # being tested.
                                           win = i, wtime = brk[i], true_lat = tlat[i])
        }
      }
    }
  }
