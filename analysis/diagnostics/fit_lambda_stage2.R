...
OUT  <- "scratch/nes_calibration/lambda_stage2_results.csv"
...
# STAGE 2 of the calibration-vs-shading grid search. Stage 1 (emission-only, 45
# configs, all 29 tags) said the SHADING parameter owns the bias, not the
# calibration: bias range 1.76 deg across lambda vs 0.76 deg across a +/-6 deg
# zenith shift, and the zero crossing sits near lambda ~3.7x TIGHTER than default --
# which independently matches the 3.1x the residual scale was measured at.
#
# But an emission optimum has already failed to transfer once this week: the
# twilight-retuned darkness regime improved emission bias 71% and made the FITS
# significantly worse (|bias| 0.734 -> 1.231, p = 0.006). Emission profile bias and
# fit bias disagreed in sign at the shipped setting. So stage 1 is a hypothesis, not
# a result, and these four fit-level points test it:
#
#   lam1.0  the shipped tangent. REPRODUCTION CONTROL -- must return 276 km mean /
#           bias -1.543, or the harness differs and nothing here is readable.
#   lam2.0  predicted better (emission bias -0.60)
#   lam4.0  predicted best   (emission bias +0.03)
#   lam0.5  predicted WORSE  (emission bias -1.73). This is the point that matters:
#           if the fits also get worse here, the emission ranking transfers and can
#           be trusted for tuning. If they get BETTER, the ranking is inverted and
#           stage 1 says nothing about fits -- which is what the darkness result hinted.
#
# delta is NOT swept at fit level: stage 1 showed its effect is small, monotone in
# the unhelpful direction, and its chosen value was unstable across all 15
# leave-one-logger-out folds, i.e. noise.
LAM_MULTS <- c(1.0, 2.0, 4.0, 0.5, 0.25, 0.125)   # 0.25/0.125 added: coverage was still climbing and accuracy had not turned over at 0.5, so the optimum sat outside the swept range on the LOOSE side -- the same edge-of-grid failure this campaign has hit three times. The script skips fits already in OUT, so only the 58 new ones run.
COMBOS <- expand.grid(area = TRUE, resp = "tangent", lam_mult = LAM_MULTS,
...
for (ci in seq_len(nrow(COMBOS))) for (tg in picked) {
  ...
  fwrite(data.table(id = tg, arm = arm, resp = sprintf("lam%.2f", COMBOS$lam_mult[ci]),
