# Does tempering the emission fix the calibration, and with it the bias?
#
# `diag_latitude_profile.R` found a reversal that nothing else explains. Per
# 12-hour window the emission is CONSERVATIVE, claiming 1.5 to 1.8 times less
# knowledge than the scatter of its own posterior means shows it has. At track
# level the posterior is OVER-confident, covering 0.72 against a nominal 0.95.
# Conservative inputs and an over-confident output can only be reconciled in how
# the windows are combined, where the engine assumes conditional independence.
#
# They are not independent: residual autocorrelation runs 0.26-0.33 over one to
# four hours and 0.26 at twenty-four. Summing the measured lags gives an
# effective sample size near a third of the count, so `temper` around 0.3 is the
# value the data imply rather than a value chosen to win.
#
# The prediction, stated before the run: tempering should widen the posterior
# and fix coverage. Whether it also moves the BIAS is the open question, and the
# answer decides whether the bias is over-confidence in disguise or something
# else entirely.
SCRATCH <- "analysis/diagnostics"   # repo-relative; run from the package root
source(file.path(SCRATCH, "nes_common.R"))
OUT  <- "scratch/nes_calibration/temper_results.csv"
...
COMBOS <- expand.grid(temper = c(1, 0.5, 0.3, 0.15),
...
# The same 8 tags as the darkness sweep, so every result on disk is comparable.
...
cat(sprintf("tangent baseline on these: %.0f km, cover %.2f\n",
            mean(tan[id %in% picked]$median_km), mean(tan[id %in% picked]$cover_lat)))
...
for (ci in seq_len(nrow(COMBOS))) for (tg in picked) {
  arm <- sprintf("%s_t%03.0f", COMBOS$resp[ci], 100 * COMBOS$temper[ci])
  ...
  if (COMBOS$resp[ci] == "tangent") {
    cal <- r$calibration
    lp <- c(lam, r$max_light, 0.10, 0.35, 2, 0.10, COMBOS$temper[ci])
  } else {
    cal <- tabs_flat[[tg]]
  ...
  t <- system.time(invisible(capture.output(
    f <- TwilightFreeGrid(g$light$time, pmax(0, g$light$light - r$baseline),
      ...
      calibration = cal, likelihood_params = lp))))[3]
  gp <- grid_posterior(f)
  ...
  fwrite(data.table(id = tg, arm = arm, resp = COMBOS$resp[ci],
                    temper = COMBOS$temper[ci], n = sum(ok),
...
o <- fread(OUT)
cat("\n=== tempering the emission ===\n")
print(as.data.frame(o[, .(tags = .N, median_km = round(mean(median_km)),
  bias_lat = round(mean(bias_lat), 2), rmse_lat = round(mean(rmse_lat), 2),
  reported_sd = round(mean(lat_sd), 2),
  cover_lat = round(mean(cover_lat), 2)), by = .(resp, temper)][order(resp, -temper)]),
  row.names = FALSE)
cat("\nPREDICTED: reported_sd rises and cover_lat approaches 0.95 as temper falls.\n")
cat("OPEN: whether bias_lat moves. If it does not, the bias is not\n")
cat("over-confidence and tempering fixes calibration only.\n")
