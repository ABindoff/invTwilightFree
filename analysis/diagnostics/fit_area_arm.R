# Does correcting the grid-cell area fix the northward pull?
#
# `diag_kernel_bias.R` isolated the defect with no data at all. The movement
# kernel is a density on the sphere evaluated on a grid uniform in degrees, and
# the code omits the cos(latitude) area of the destination cell, so every cell
# counts equally and the smaller high-latitude cells collect weight they have not
# earned. Incoming mass tracks 1/cos(latitude) at r = 0.9999 and is 2.32 times
# larger at 68 N than 22 N. Between 38 N and 50 N, where these animals live, that
# is 0.204 nats PER STEP toward the pole.
#
# It predicts what nothing else did: a northward pull present in EVERY response
# already scored, of +0.53 to +2.78 degrees, sitting between what the emission
# prefers and where the track lands, positive in all four arms whichever way
# their emissions lean. And it explains why tempering the light drove the fitted
# latitude north past the middle of the search domain to 51.
#
# Two responses, correction off and on. `off` must reproduce the archived
# numbers to the kilometre, which is the check that the flag is inert when
# false; `on` is the fix.
SCRATCH <- "analysis/diagnostics"   # repo-relative; run from the package root
source(file.path(SCRATCH, "nes_common.R"))
OUT  <- "scratch/nes_calibration/area_results.csv"
...
COMBOS <- expand.grid(area = c(FALSE, TRUE),
                      resp = c("tangent", "dark"), stringsAsFactors = FALSE)
DARK_FRAC <- 0.35; DARK_RATIO <- 100; DARK_PSLAB <- 0.35
...
cat(sprintf("archived tangent on these: %.0f km, bias %+.2f, cover %.2f\n",
            mean(tan[id %in% picked]$median_km),
            mean(tan[id %in% picked]$bias_lat),
            mean(tan[id %in% picked]$cover_lat)))
...
o <- fread(OUT)
cat("\n=== grid-cell area correction ===\n")
print(as.data.frame(o[, .(tags = .N, median_km = round(mean(median_km)),
  bias_lat = round(mean(bias_lat), 2), rmse_lat = round(mean(rmse_lat), 2),
  cover_lat = round(mean(cover_lat), 2)), by = .(resp, area)][order(resp, area)]),
  row.names = FALSE)
w <- dcast(o, id + resp ~ area, value.var = c("median_km", "bias_lat", "cover_lat"))
for (rr in unique(w$resp)) {
  x <- w[resp == rr]
  cat(sprintf("\n%s: bias %+.2f -> %+.2f, km %.0f -> %.0f, cover %.2f -> %.2f\n",
              rr, mean(x$bias_lat_FALSE), mean(x$bias_lat_TRUE),
              mean(x$median_km_FALSE), mean(x$median_km_TRUE),
              mean(x$cover_lat_FALSE), mean(x$cover_lat_TRUE)))
  cat(sprintf("   better on %d of %d tags, wilcoxon p = %.4f\n",
              sum(x$median_km_TRUE < x$median_km_FALSE), nrow(x),
              suppressWarnings(wilcox.test(x$median_km_TRUE, x$median_km_FALSE,
                                           paired = TRUE)$p.value)))
}
cat("\nPREDICTED: bias falls by roughly the pull, +1.1 for tangent and +1.3 for\n")
cat("dark. The OFF rows must match the archived numbers, or the flag is not\n")
cat("inert when false and nothing here can be trusted.\n")
