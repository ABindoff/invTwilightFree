...
md_path <- "scratch/diffusion_optimization_ffbs.md"
...
# Initialise Markdown
cat("# Diffusion Parameter Optimization Log (FFBS)\n\n", file = md_path)
...
cat("Starting FFBS grid search on IDEAL scenario...\n")
...
for (d1 in d1_grid) {
  for (d2 in d2_grid) {
    ...
    errs <- get_dist(fit$lon, safe_lat, t_lon, t_lat)
