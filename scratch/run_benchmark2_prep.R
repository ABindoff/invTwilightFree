...
run_scenario <- function(light_col, scenario_name) {
  ...
  cat("Found", nrow(twl), "twilights.\n")
  
  # 2. FLightR
  cat("Running FLightR...\n")
  time_flightr <- system.time({
    flightr_data <- data.frame(
      datetime = format(date_time, "%Y-%m-%dT%H:%M:%SZ"),
      light = light,
      twilight = 0,
    ...
    # 7-day calibration
    calib_df <- data.frame(calibration.start = date_time[1], calibration.stop = date_time[1] + 7*86400, lon = track_all$true_lon[1], lat = track_all$true_lat[1])
    ...
    fit_flightr <- FLightR::run.particle.filter(prerun, nParticles = 30000, plot = FALSE)
  })

  saveRDS(list(fit = fit_flightr, time = time_flightr), paste0("vignettes/cache_flightr", prefix, ".rds"))
  
  # 3. SGAT (Skipped as per user request to save time and avoid catastrophic failure)
  # cat("Running SGAT...\n")
  ...
  #   model <- SGAT::thresholdModel(twl$Twilight, twl$Rise, twilight.model = "Normal", alpha = alpha_mat, beta = c(2, 2), x0 = x0, z0 = z0)
...
# Execute
# run_scenario("light_ideal", "cloudy")
